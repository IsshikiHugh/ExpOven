...
def test_progress_without_notifications():
    ...
    for i in oven.progress_range(10, desc="Test range", enable_notifications=False):
        time.sleep(0.1)
    ...
    for item in oven.progress(data, desc="Test iterable", enable_notifications=False):
        time.sleep(0.1)
    print("✓ progress with iterable works")
    
    # Test manual progress
    print("Testing manual progress...")
    with oven.ProgressBar(total=10, desc="Manual test", enable_notifications=False) as pbar:
        for i in range(10):
            time.sleep(0.1)
            pbar.update(1)
...
def test_progress_with_notifications():
    ...
    try:
        ...
        for i in oven.progress_range(5, desc="Notification test", notify_interval=1.0):
            time.sleep(0.3)
        print("✓ Progress with notifications works (or notifications disabled due to config)")
    except Exception as e:
        print(f"⚠ Progress with notifications failed (expected if not configured): {e}")
...
def test_error_handling():
    ...
    try:
        with oven.ProgressBar(total=5, desc="Error test", enable_notifications=False) as pbar:
            for i in range(5):
                time.sleep(0.1)
                pbar.update(1)
                if i == 3:
