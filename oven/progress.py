...
from oven.backends.api import Signal, ExpInfoBase
...
class ProgressBar:
    ...
    def _setup_oven_integration(self):
        ...
        try:
            ...
            if oven:
                ...
                meta['cmd'] = f'Progress: {self.desc}' if self.desc else 'Progress'
    ...
    def _format_progress_description(self) -> str:
        ...
        return (f"{self.desc}: {percentage:.1f}% ({self.n}/{self.total}) "
    ...
    def _start_notify_thread(self):
        ...
        self._notify_thread = threading.Thread(target=notify_worker, daemon=True)
    ...
    def _send_progress_notification(self):
        ...
        time_threshold_met = (current_time - self.last_notify_time) >= self.notify_interval
        progress_threshold_met = abs(current_progress - self.last_notify_progress) >= self.notify_threshold
    ...
    def _display_progress(self):
        ...
        if self.total:
            ...
            print(f'\r{self.desc}: {percentage:6.2f}%|{bar}| {self.n}/{self.total} '
                  f'[{self._format_time(elapsed)}, {rate:.2f}{self.unit}/s]', end='', flush=True)
        else:
            elapsed = time.time() - self.start_time
            rate = self.n / elapsed if elapsed > 0 else 0
            print(f'\r{self.desc}: {self.n}{self.unit} [{self._format_time(elapsed)}, {rate:.2f}{self.unit}/s]', 
                  end='', flush=True)
        ...
        if self.enable_notifications and self.exp_info:
            try:
                ...
                if self.total and self.n >= self.total:
                    self.exp_info.update_signal(signal=Signal.T, description="Progress completed!")
                else:
                    self.exp_info.update_signal(signal=Signal.T, description=self._format_progress_description())
            except Exception as e:
...
def progress(iterable=None, desc="", total=None, leave=True, file=None, ncols=None, 
            ...
            write_bytes=None, lock_args=None, nrows=None, colour=None, delay=0, gui=False,
            # ExpOven specific
            notify_interval=30.0, notify_mode="http", notify_threshold=0.05, 
            enable_notifications=True, **kwargs):
    """
    ...
    """
    return ProgressBar(
        iterable=iterable, desc=desc, total=total, leave=leave, file=file, ncols=ncols,
        mininterval=mininterval, maxinterval=maxinterval, miniters=miniters, ascii=ascii,
        disable=disable, unit=unit, unit_scale=unit_scale, dynamic_ncols=dynamic_ncols,
        smoothing=smoothing, bar_format=bar_format, initial=initial, position=position,
        postfix=postfix, unit_divisor=unit_divisor, write_bytes=write_bytes,
        lock_args=lock_args, nrows=nrows, colour=colour, delay=delay, gui=gui,
        notify_interval=notify_interval, notify_mode=notify_mode, 
        notify_threshold=notify_threshold, enable_notifications=enable_notifications,
        **kwargs
    )
